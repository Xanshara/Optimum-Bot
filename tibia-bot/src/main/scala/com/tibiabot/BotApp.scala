...
object BotApp extends App with StrictLogging {
  ...
  logger.info("🗳️ Initializing Poll System...")
  ...
  .enableCache(CacheFlag.ONLINE_STATUS)       // ← KLUCZOWE!
...
logger.info("✅ Nickname update scheduler started (interval: 12h)")
...
private val serverStatsCommand: SlashCommandData = Commands.slash("serverstats", "Zarządzaj statystykami serwera")
  .setDefaultPermissions(DefaultMemberPermissions.enabledFor(Permission.MANAGE_SERVER))
  .addSubcommands(
    new SubcommandData("on", "Włącz wyświetlanie statystyk serwera"),
    new SubcommandData("off", "Wyłącz i usuń wszystkie statystyki serwera"),
    new SubcommandData("configure", "Zarządzaj statystykami publicznymi")
      .addOptions(
    ...
    new SubcommandData("admin", "Zarządzaj statystykami adminowymi")
      .addOptions(
        new OptionData(OptionType.STRING, "action", "Akcja do wykonania").setRequired(true)
          .addChoices(
            new Choice("add", "add"),
    ...
    new SubcommandData("list", "Wyświetl wszystkie dostępne statystyki")
  )
...
// postac_info command
private val postacInfoCommand: SlashCommandData = Commands.slash("postac_info", "Wyświetla szczegółowe informacje o postaci z Tibii")
  .addOption(OptionType.STRING, "nick", "Nazwa postaci do sprawdzenia", true)

private val changeNickCommand: SlashCommandData = Commands.slash("changenick", "Zarządzaj nickami członków z weryfikacją w Tibii")
  .setDefaultPermissions(DefaultMemberPermissions.enabledFor(Permission.MANAGE_SERVER))
  .setGuildOnly(true)
  .addSubcommands(
    new SubcommandData("set", "Zmień nick członka")
      .addOption(OptionType.STRING, "old_nick", "Obecny nick członka na Discordzie", true)
      .addOption(OptionType.STRING, "new_name", "Nowa nazwa (postać z Tibii)", true),
    new SubcommandData("refresh", "Odśwież levele wszystkich członków")
  )

private val radioCommand: SlashCommandData = Commands.slash("radio", "Włącz/wyłącz Radio na wybranym kanale")
  .addOption(OptionType.STRING, "action", "Akcja: on lub off", true, true)
  .addOptions(
    new OptionData(OptionType.CHANNEL, "channel", "Kanał głosowy (wymagane dla 'on')", false)
      .setChannelTypes(ChannelType.VOICE, ChannelType.STAGE)
  )
  .addOption(OptionType.STRING, "url", "URL streamu (opcjonalnie, domyślnie RMF FM)", false)
  ...
  guilds.foreach{g =>
    ...
    if (g.getIdLong == 1340737877058785352L) { // Optimum Bot Discord
      ...
      g.updateCommands().addCommands(adminCommands.asJava).queue(
        _ => {
          logger.info(s"✅ Admin commands registered for ${g.getName}")
          // Verify reactionrole is registered
          g.retrieveCommands().queue(cmds => {
            val hasRR = cmds.asScala.exists(_.getName == "reactionrole")
            if (hasRR) logger.info(s"✅✅ /reactionrole confirmed on ${g.getName}")
            else logger.warn(s"⚠️ /reactionrole NOT FOUND on ${g.getName}!")
          })
        },
        error => logger.error(s"❌ Failed to register admin commands for ${g.getName}", error)
      )
    } else {
      // update the commands
      logger.info(s"🔧 Registering ${commands.size} commands (including /reactionrole)...")
      g.updateCommands().addCommands(commands.asJava).queue(
        _ => {
          logger.info(s"✅ Commands registered for ${g.getName}")
          // Verify reactionrole is registered
          g.retrieveCommands().queue(cmds => {
            val hasRR = cmds.asScala.exists(_.getName == "reactionrole")
            if (hasRR) logger.info(s"✅✅ /reactionrole confirmed on ${g.getName}")
        ...
        error => logger.error(s"❌ Failed to register commands for ${g.getName}", error)
      )
    }
  }
...
guilds.foreach { guild =>
  try {
    ...
    // Sprawdź czy baza danych istnieje dla tej gildii
    if (checkConfigDatabase(guild)) {
      
      // 1. Załaduj konfigurację światów z bazy
      val worldsInfo = worldConfig(guild)
      if (worldsInfo.nonEmpty) {
        worldsData += (guildId -> worldsInfo)
        logger.info(s"✅ Loaded ${worldsInfo.length} worlds for ${guild.getName}: ${worldsInfo.map(_.name).mkString(", ")}")
      } else {
        logger.info(s"ℹ️ No worlds configured for ${guild.getName}")
      }
      
      // 2. Załaduj listę huntowanych graczy z bazy
      val huntedPlayers = playerConfig(guild, "hunted_players")
      if (huntedPlayers.nonEmpty) {
        huntedPlayersData += (guildId -> huntedPlayers)
        logger.info(s"✅ Loaded ${huntedPlayers.length} hunted players for ${guild.getName}")
      }
      
      // 3. Załaduj listę allied graczy z bazy
      val alliedPlayers = playerConfig(guild, "allied_players")
      if (alliedPlayers.nonEmpty) {
        alliedPlayersData += (guildId -> alliedPlayers)
        logger.info(s"✅ Loaded ${alliedPlayers.length} allied players for ${guild.getName}")
      }
      
      // 4. Załaduj listę huntowanych gildii z bazy
      val huntedGuilds = guildConfig(guild, "hunted_guilds")
      if (huntedGuilds.nonEmpty) {
        huntedGuildsData += (guildId -> huntedGuilds)
        logger.info(s"✅ Loaded ${huntedGuilds.length} hunted guilds for ${guild.getName}")
      }
      
      // 5. Załaduj listę allied gildii z bazy
      val alliedGuilds = guildConfig(guild, "allied_guilds")
      if (alliedGuilds.nonEmpty) {
        alliedGuildsData += (guildId -> alliedGuilds)
        logger.info(s"✅ Loaded ${alliedGuilds.length} allied guilds for ${guild.getName}")
      }
      
      // 6. Załaduj tracked activity characters z bazy
      val activityInfo = activityConfig(guild, "tracked_activity")
      if (activityInfo.nonEmpty) {
        activityData += (guildId -> activityInfo)
        logger.info(s"✅ Loaded ${activityInfo.length} tracked activity characters for ${guild.getName}")
      }
      
      // 7. Załaduj custom sort data z bazy
      ...
      if (customSortInfo.nonEmpty) {
        ...
        logger.info(s"✅ Loaded ${customSortInfo.length} custom sort entries for ${guild.getName}")
      }
      
      // 8. Ustaw domyślny stan activityCommandBlocker
      activityCommandBlocker += (guildId -> false)
      
      // 9. Załaduj konfigurację blacklist
      blacklistManager.loadFromDatabase(guild)
      logger.info(s"✅ Loaded blacklist configuration for ${guild.getName}")
      
      logger.info(s"✅✅ Successfully loaded all data for ${guild.getName}")
      
    } else {
      logger.info(s"ℹ️ No database configured for ${guild.getName}")
    }
    
  } catch {
    case ex: Exception =>
      logger.error(s"❌ Failed to load data for guild ${guild.getName} (${guild.getId})", ex)
...
// startBot(None, None) ustawia to automatycznie na końcu
